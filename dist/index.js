...
var ReactEditor = {
  ...
  deselect: function deselect(editor) {
    ...
    if (!root) {
      editor.onError({
        ...
        message: 'Cannot deselect the editor because its document is not defined.'
      });
      return;
    }
  ...
  findDocumentOrShadowRoot: function findDocumentOrShadowRoot(editor) {
    ...
    if (!el) {
      return editor.onError({
        ...
        message: 'Cannot find the document or ShadowRoot because the editor has no DOM node.'
      });
    }
  ...
  findEventRange: function findEventRange(editor, event) {
    ...
    if (x == null || y == null) {
      return editor.onError({
        ...
        message: "Cannot resolve a Slate range from a DOM event: ".concat(event),
        data: {
          event: event
        }
      });
  ...
  findPath: function findPath(editor, node) {
    ...
    return editor.onError({
      ...
      message: "Unable to find the path for Slate node: ".concat(slate.Scrubber.stringify(node))
    });
  },
  ...
  toDOMNode: function toDOMNode(editor, node) {
    ...
    if (!domNode) {
      return editor.onError({
        ...
        message: "Cannot resolve a DOM node from Slate node: ".concat(slate.Scrubber.stringify(node)),
        data: {
          node: node
        }
      });
  ...
  toDOMPoint: function toDOMPoint(editor, point) {
    var entry = slate.Editor.node(editor, point.path);

    if (!entry) {
      return editor.onError({
        key: 'ReactEditor.toDOMPoint.node',
        message: "Cannot resolve a DOM point from Slate point: ".concat(slate.Scrubber.stringify(point)),
        data: {
          point: point
        }
      });
  ...
  toDOMRange: function toDOMRange(editor, range) {
    ...
    if (!domAnchor) {
      return editor.onError({
        ...
        message: "Cannot resolve a DOM point from Slate point: ".concat(slate.Scrubber.stringify(anchor)),
        data: {
          point: anchor
        }
      });
    ...
    if (!domFocus) {
      return editor.onError({
        ...
        message: "Cannot resolve a DOM point from Slate point: ".concat(slate.Scrubber.stringify(focus)),
        data: {
          point: focus
        }
      });
  ...
  toSlateNode: function toSlateNode(editor, domNode) {
    ...
    if (!node) {
      return editor.onError({
        ...
        message: "Cannot resolve a Slate node from DOM node: ".concat(domEl),
        data: {
  ...
  toSlatePoint: function toSlatePoint(editor, domPoint, options) {
    ...
    if (parentNode) {
      ...
      if (!editorEl) {
        return editor.onError({
          ...
          message: "Cannot resolve a Slate point from DOM point: ".concat(domPoint),
          data: {
            domPoint: domPoint
          }
        });
      ...
      if (node && ReactEditor.hasDOMNode(editor, node, {
        ...
        if (!_slateNode) {
          return editor.onError({
            ...
            message: "Cannot resolve a Slate node from DOM node: ".concat(node),
            data: {
              node: node
            }
          });
        ...
        var slatePath = ReactEditor.findPath(editor, _slateNode);

        if (!slatePath) {
          return editor.onError({
            key: 'ReactEditor.toSlatePoint.findPath',
            message: "Cannot resolve a Slate path from DOM node: ".concat(_slateNode),
            data: {
              slateNode: _slateNode
            }
          });
        }

        var start = slate.Editor.start(editor, slatePath);

        if (!start) {
          return editor.onError({
            key: 'ReactEditor.toSlatePoint.start',
            message: "Cannot resolve a Slate point from DOM node: ".concat(node),
            data: {
              at: slatePath
            }
          });
    ...
    if (!slateNode) {
      return editor.onError({
        ...
        message: "Cannot resolve a Slate node from DOM node: ".concat(textNode),
        data: {
          textNode: textNode
        }
      });
  ...
  toSlateRange: function toSlateRange(editor, domRange, options) {
    ...
    if (anchorNode == null || focusNode == null || anchorOffset == null || focusOffset == null) {
      return editor.onError({
        ...
        message: "Cannot resolve a Slate range from DOM range: ".concat(domRange),
        data: {
          domRange: domRange
        }
      });
...
function verifyDiffState(editor, textDiff) {
  ...
  if (!nextPath) {
    return editor.onError({
      key: 'diff-text.next',
      message: "Cannot get next path from path ".concat(slate.Scrubber.stringify(path)),
      data: {
        path: path
      },
      recovery: false
...
function createAndroidInputManager(_ref) {
    ...
    if (action.at) {
      ...
      if (!_targetRange) {
        editor.onError({
          ...
          message: "Failed to create range for ".concat(slate.Scrubber.stringify(target)),
          data: {
            target: target
          }
        });
  ...
  var storeDiff = function storeDiff(path, diff) {
    ...
    if (!target) {
      editor.onError({
        ...
        message: "Failed to find leaf node at path: ".concat(path),
        data: {
          path: path
        }
      });
  ...
  var handleDOMBeforeInput = function handleDOMBeforeInput(event) {
    ...
    if (nativeTargetRange) {
      var range = ReactEditor.toSlateRange(editor, nativeTargetRange, {
        exactMatch: false,
        suppressThrow: true
      });
      ...
      if (!range) {
        editor.onError({
          key: 'createAndroidInputManager.handleDOMBeforeInput.toSlateRange.nativeTargetRange',
          message: "Failed to resolve Slate range from DOM range: ".concat(nativeTargetRange),
          data: {
            nativeTargetRange: nativeTargetRange
          }
        });
        return;
      }
    ...
    if (!targetRange && domSelection) {
      ...
      var _range = ReactEditor.toSlateRange(editor, domSelection, {
        exactMatch: false,
        suppressThrow: true
      });
      ...
      if (!_range) {
        editor.onError({
          key: 'createAndroidInputManager.handleDOMBeforeInput.toSlateRange.domSelection',
          message: "Failed to resolve Slate range from DOM range: ".concat(domSelection),
          data: {
    ...
    if (type.startsWith('delete')) {
      if (slate.Range.isExpanded(targetRange)) {
        ...
        if (!_leaf) {
          editor.onError({
            ...
            message: "Failed to find leaf node at path: ".concat(_start.path),
            data: {
              path: _start.path
            }
          });
      ...
      var entry = slate.Editor.leaf(editor, start.path);

      if (!entry) {
        editor.onError({
          key: 'createAndroidInputManager.handleDOMBeforeInput.leaf.2',
          message: "Failed to find leaf node at path: ".concat(start.path),
          data: {
            path: start.path
          }
        });
      ...
      if (slate.Range.isExpanded(targetRange)) {
        if (canStoreDiff && slate.Path.equals(targetRange.anchor.path, targetRange.focus.path)) {
          ...
          var _range2 = slate.Editor.range(editor, point, point);

          if (!_range2) {
            editor.onError({
              key: 'createAndroidInputManager.handleDOMBeforeInput.range',
              message: "Failed to create range at point: ".concat(point),
              data: {
                point: point
              }
            });
      ...
      case 'deleteContentForward':
        {
          ...
          if (canStoreDiff && slate.Range.isCollapsed(targetRange)) {
            ...
            if (!targetNode) {
              return editor.onError({
                key: 'createAndroidInputManager.handleDOMBeforeInput.deleteContent.leaf',
                message: "Failed to find leaf node at path: ".concat(anchor.path),
                data: {
                  path: anchor.path
                }
              });
  ...
  useIsomorphicLayoutEffect(function () {
    ...
    var setDomSelection = function setDomSelection(forceChange) {
      ...
      if (selection && !ReactEditor.hasRange(editor, selection)) {
        var _ReactEditor$toSlateR;

        editor.selection = (_ReactEditor$toSlateR = ReactEditor.toSlateRange(editor, domSelection, {
          exactMatch: false,
          suppressThrow: true
        })) !== null && _ReactEditor$toSlateR !== void 0 ? _ReactEditor$toSlateR : null;
        return;
  ...
  var onDOMBeforeInput = React.useCallback(function (event) {
    ...
    if (!readOnly && ReactEditor.hasEditableTarget(editor, event.target) && !isDOMEventHandled(event, propsOnDOMBeforeInput)) {
      ...
      selection.anchor.offset !== 0) {
        ...
        var point = ReactEditor.toDOMPoint(editor, anchor);
      ...
      if (ReactEditor.hasTarget(editor, event.target) && !isEventHandled(event, attributes.onClick) && isDOMNode(event.target)) {
        ...
        if (event.detail === TRIPLE_CLICK && path.length >= 1) {
          ...
          if (!range) {
            editor.onError({
              ...
              message: 'Cannot resolve a Slate range from a Slate path',
              data: {
                path: blockPath
              }
            });
        ...
        if (startVoid && endVoid && slate.Path.equals(startVoid[1], endVoid[1])) {
          ...
          if (!_range) {
            editor.onError({
              key: 'Editable.onClick.range',
              message: 'Cannot resolve a Slate range from a Slate point',
              data: {
                point: _start
              }
            });
      ...
      if (!readOnly && !state.isUpdatingSelection && ReactEditor.hasEditableTarget(editor, event.target) && !isEventHandled(event, attributes.onFocus)) {
        ...
        if (!el) {
          editor.onError({
            ...
            message: 'Cannot resolve a native DOM node from the editor'
          });
          return;
        }
        ...
        if (!root) {
          editor.onError({
            ...
            message: 'Cannot resolve a native Document or ShadowRoot from the editor'
          });
          return;
        }
...
var areRangesSameLine = function areRangesSameLine(editor, range1, range2) {
  var _ReactEditor$toDOMRan, _ReactEditor$toDOMRan2;

  var rect1 = (_ReactEditor$toDOMRan = ReactEditor.toDOMRange(editor, range1)) === null || _ReactEditor$toDOMRan === void 0 ? void 0 : _ReactEditor$toDOMRan.getBoundingClientRect();
  if (!rect1) return false;
  var rect2 = (_ReactEditor$toDOMRan2 = ReactEditor.toDOMRange(editor, range2)) === null || _ReactEditor$toDOMRan2 === void 0 ? void 0 : _ReactEditor$toDOMRan2.getBoundingClientRect();
...
var findCurrentLineRange = function findCurrentLineRange(editor, parentRange) {
  ...
  var leftRange = slate.Editor.range(editor, positions[left]);
  if (!leftRange) return;
  ...
  while (middle !== positions.length && middle !== left) {
    var middleRange = slate.Editor.range(editor, positions[middle]);
...
var withReact = function withReact(editor) {
      ...
      if (parentBlockEntry) {
        ...
        if (!parentElementRange) {
          editor.onError({
            ...
            data: {
              parentBlockPath: parentBlockPath
            }
          });
          return;
        }
        ...
        if (currentLineRange && !slate.Range.isCollapsed(currentLineRange)) {
  ...
  e.apply = function (op) {
    ...
    switch (op.type) {
      ...
      case 'remove_node':
        {
          var parentPath = slate.Path.parent(op.path);
          ...
          matches.push.apply(matches, _toConsumableArray(getMatches(e, parentPath)));
          break;
        }
      ...
      case 'move_node':
        {
          var _parentPath = slate.Path.parent(op.path);

          var parentNewPath = slate.Path.parent(op.newPath);

          if (!_parentPath || !parentNewPath) {
            editor.onError({
          ...
          var commonPath = slate.Path.common(_parentPath, parentNewPath);
    ...
    for (var _i = 0, _matches = matches; _i < _matches.length; _i++) {
      ...
      var entry = slate.Editor.node(e, path);
