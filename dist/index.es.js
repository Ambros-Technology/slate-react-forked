...
var ReactEditor = {
  ...
  findPath: (editor, node) => {
    ...
    return editor.onError({
      ...
      message: "Unable to find the path for Slate node: ".concat(Scrubber.stringify(node))
    });
  },
  focus: editor => {
    var el = ReactEditor.toDOMNode(editor, editor);
  ...
  toDOMNode: (editor, node) => {
    ...
    if (!domNode) {
      return editor.onError({
        ...
        message: "Cannot resolve a DOM node from Slate node: ".concat(Scrubber.stringify(node)),
        data: {
          node
        }
      });
  ...
  toDOMPoint: (editor, point) => {
    var entry = Editor.node(editor, point.path);

    if (!entry) {
      return editor.onError({
        key: 'ReactEditor.toDOMPoint.node',
        message: "Cannot resolve a DOM point from Slate point: ".concat(Scrubber.stringify(point)),
        data: {
          point
        }
      });
  ...
  toDOMRange: (editor, range) => {
    ...
    if (!domAnchor) {
      return editor.onError({
        ...
        message: "Cannot resolve a DOM point from Slate point: ".concat(Scrubber.stringify(anchor)),
        data: {
          point: anchor
        }
      });
    ...
    if (!domFocus) {
      return editor.onError({
        ...
        message: "Cannot resolve a DOM point from Slate point: ".concat(Scrubber.stringify(focus)),
        data: {
          point: focus
        }
      });
  ...
  toSlatePoint: (editor, domPoint, options) => {
    ...
    if (IS_ANDROID && !textNode && !exactMatch) {
      ...
      if (node && ReactEditor.hasDOMNode(editor, node, {
        ...
        var start = Editor.start(editor, slatePath);
...
function verifyDiffState(editor, textDiff) {
  ...
  if (!nextPath) {
    return editor.onError({
      key: 'diff-text.next',
      message: "Cannot get next path from path ".concat(Scrubber.stringify(path)),
      data: {
        path
      },
      recovery: false
...
function createAndroidInputManager(_ref) {
    ...
    if (action.at) {
      ...
      if (!_targetRange) {
        editor.onError({
          ...
          message: "Failed to create range for ".concat(Scrubber.stringify(target)),
          data: {
            target
          }
        });
  ...
  var handleDOMBeforeInput = event => {
    ...
    if (type.startsWith('delete')) {
      ...
      var entry = Editor.leaf(editor, start.path);
      ...
      if (Range.isExpanded(targetRange)) {
        if (canStoreDiff && Path.equals(targetRange.anchor.path, targetRange.focus.path)) {
          ...
          var _range2 = Editor.range(editor, point, point);
...
var findCurrentLineRange = (editor, parentRange) => {
  ...
  var leftRange = Editor.range(editor, positions[left]);
  if (!leftRange) return;
  ...
  while (middle !== positions.length && middle !== left) {
    var middleRange = Editor.range(editor, positions[middle]);
...
var withReact = function withReact(editor) {
  ...
  e.apply = op => {
    ...
    switch (op.type) {
      ...
      case 'remove_node':
        {
          var parentPath = Path.parent(op.path);
      ...
      case 'move_node':
        {
          var _parentPath = Path.parent(op.path);

          var parentNewPath = Path.parent(op.newPath);
          ...
          var commonPath = Path.common(_parentPath, parentNewPath);
